...
# Use the official Ruby 3.2.2 Slim Bullseye image as base:
...
RUN userhome=$(eval echo ~${DEVELOPER_USERNAME}) \
    ...
    && mkdir -p /workspaces/rails-vue-docker-vscode \
    && chown -R ${DEVELOPER_USERNAME}:${DEVELOPER_USERNAME} /workspaces/rails-vue-docker-vscode

# Add the app's "bin/" directory to PATH:
ENV PATH=/workspaces/rails-vue-docker-vscode/bin:$PATH

# Set the app path as the working directory:
WORKDIR /workspaces/rails-vue-docker-vscode
...
COPY --chown=${DEVELOPER_USERNAME} Gemfile* /workspaces/rails-vue-docker-vscode/
...
COPY --from=testing /workspaces/rails-vue-docker-vscode/ /workspaces/rails-vue-docker-vscode/
...
RUN git config --global --add safe.directory /workspaces/rails-vue-docker-vscode
...
COPY --chown=${DEVELOPER_USERNAME} . /workspaces/rails-vue-docker-vscode/
...
# final destination at /workspaces/rails-vue-docker-vscode:
COPY --from=builder --chown=nobody:nogroup /workspaces/rails-vue-docker-vscode /workspaces/rails-vue-docker-vscode

# Set the container user to 'nobody':
USER nobody

# Set the RAILS and PORT default values:
ENV HOME=/workspaces/rails-vue-docker-vscode \
...
ENTRYPOINT [ "/workspaces/rails-vue-docker-vscode/bin/entrypoint" ]
